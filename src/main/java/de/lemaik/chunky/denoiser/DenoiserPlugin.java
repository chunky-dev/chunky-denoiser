...
public class DenoiserPlugin implements Plugin {
  ...
  public void attach(Chunky chunky) {
    ...
    try {
      ...
      f.set(chunky,
          (RendererFactory) (context, headless) -> new BetterRenderManager(context, headless,
              rayTracer));
    ...
    chunky.setRenderControlsTabTransformer(tabs -> {
      ...
      return prev.apply(tabs);
    });

    TabTransformer mainPrev = chunky.getMainTabTransformer();
    chunky.setMainTabTransformer(tabs -> {
      PreviewTab previewTab = new PreviewTab();
      tabs.add(previewTab.getTab());
      previewTab.setController(chunky.getRenderController());
      return mainPrev.apply(tabs);
    });
