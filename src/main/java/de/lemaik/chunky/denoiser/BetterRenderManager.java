...
public class BetterRenderManager extends DefaultRenderManager {
    ...
    private void writePfmImage(OutputStream out, boolean postProcess) throws IOException {
        ...
        final int width = ChunkyCompatHelper.Scene.getCanvasWidth(scene);
        final int height = ChunkyCompatHelper.Scene.getCanvasHeight(scene);
                ...
                if (postProcess) {
                    ...
                    if (filter instanceof PixelPostProcessingFilter) {
                        ((PixelPostProcessingFilter) filter)
                                .processPixel(width, height, samples, x, y, scene.getExposure(),
                                        result);
                    }
                } else {
                    result[0] = samples[(y * width + x) * 3 + 0];
                    result[1] = samples[(y * width + x) * 3 + 1];
                    result[2] = samples[(y * width + x) * 3 + 2];
                }
                pixels[(y * width + x) * 3] = Math.min(1.0, result[0]);
                pixels[(y * width + x) * 3 + 1] = Math.min(1.0, result[1]);
                pixels[(y * width + x) * 3 + 2] = Math.min(1.0, result[2]);
            }
        }

        PortableFloatMap.writeImage(pixels, width, height, ByteOrder.LITTLE_ENDIAN, out);
    }
    ...
    private void writeNormalPfmImage(OutputStream out) throws IOException {
        ...
        PortableFloatMap.writeImage(pixels, ChunkyCompatHelper.Scene.getCanvasWidth(scene), ChunkyCompatHelper.Scene.getCanvasHeight(scene), ByteOrder.LITTLE_ENDIAN, out);
    }
}
